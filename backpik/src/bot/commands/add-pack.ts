...
export default async function addPack(
  ...
  if (!metadata.sourceUrl) {
    await client.replyNotice(roomId, event, "Don't have about in this sticker!")
    return
  }
  ...
  if (packSource.origin === new URL(APP_URL).origin) {
    ...
    if (!pack) {
      await client.replyNotice(roomId, event, "Strange.. I don'r rememver this pack. Sorry :(")
      return
    }
    ...
    await client.replyNotice(roomId, event, 'Downloading stickers from other Pik instances is not implemented yet')
  }
}
